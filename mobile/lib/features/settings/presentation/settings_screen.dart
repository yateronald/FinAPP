...
class SettingsScreen extends ConsumerWidget {
        ...
        children: [
          ...
          _card(context, [
            ...
            _switchTile(
              ...
              (v) => _toggle(ref, 'emailNotifications', v),
            ),
            _divider(context),
            _switchTile(
              context,
              ...
              (v) async {
                if (v) {
                  ...
                  await disableAi(context, ref);
                }
              },
              subtitle: settings?.aiEnabled == true
                  ? t.aiOnSubtitle
                  : t.aiOffSubtitle,
            ),
          ]),
  ...
  Widget _selectorTile(BuildContext context, String label, Widget selector) =>
      Padding(
...
class _AiSettingsCardState extends ConsumerState<_AiSettingsCard> {
  ...
  String _savedProvider(UserSettings? s) =>
      (s?.aiProvider ?? 'GEMINI').toUpperCase();
  ...
  Future<void> _save(String provider, String model) async {
    ...
    final t = context.t;
    try {
      final key = provider == 'AGENTROUTER'
          ? 'agentRouterModel'
          : 'geminiModel';
      await ref.read(settingsRepositoryProvider).updateSettings({
        'aiProvider': provider,
